...
const [info, setInfo] = useState({
  ...
  let portfoliosCounter = 0;
    let date = Date.now();
  const users = props.adminData.userArray.map((users) => {
    console.log(users);
    userCounter++;
    return (
  ...
  const portfolios = props.adminData.portfolioArray.map((portfolio) => {
    portfoliosCounter++;
    return (
      <div className="user-list">
        ...
        <h3 className="users-info">
          {" "}
          Name : {portfolio.name} <br /> Total Value : {portfolio.total_value}{" "}
        </h3>
      </div>
    );
  });

  const competitions = props.adminData.competitionArray.map((competitions) => {
